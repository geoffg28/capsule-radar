...
// localtime_r is POSIX. The ESP32 (newlib) and macOS have it, but the Windows CRT ships
// localtime_s instead, with the arguments the other way round — and the desktop simulator
// builds on Windows too, so go through this shim rather than calling localtime_r directly.
static inline struct tm *localtime_portable(const time_t *t, struct tm *out) {
#if defined(_WIN32)
...
static void build_weather(void) {
    ...
    if (haveImage && pixels && s_wxCanvas) {
        ...
        if (frameTime && localtime_portable(&ft, &ti)) snprintf(stamp, sizeof(stamp), "%02d:%02d", ti.tm_hour, ti.tm_min);
        char attr[64];
...
static void build_card(void) {
    ...
    // large text needs a wider, taller card (the 18px data lines under the title). No route
    // line here — the origin/destination lookup was removed, so the card is shorter than upstream's.
    lv_obj_set_size(s_card, s_bigText ? 316 : 300, s_bigText ? 120 : 90);
...
void ui_create(void) {
    ...
    lv_obj_set_style_text_font(s_zoomLbl, F14(), 0);         // F14(): honours the large-text setting
